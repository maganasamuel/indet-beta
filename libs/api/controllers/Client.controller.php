...
class ClientController extends Database
{
    ...
    public function getClientsAssignedToAdviser($adviser_id = "", $requesting = "Adviser")
    {
        ...
        $query = "Select c.*, a.name as adviser_name, a.email as adviser_email, l.name as leadgen_name, IFNULL((SELECT instructions FROM onlinei1_appointment_setter.appointments WHERE onlinei1_appointment_setter.appointments.indet_id IN (SELECT c.id FROM clients_tbl) LIMIT 1),'N/A') AS instructions,IFNULL((SELECT additional_notes FROM onlinei1_appointment_setter.appointments WHERE onlinei1_appointment_setter.appointments.indet_id IN (SELECT c.id FROM clients_tbl) LIMIT 1),'N/A') AS additional_notes from clients_tbl c LEFT JOIN adviser_tbl a ON c.assigned_to = a.id LEFT JOIN leadgen_tbl l ON l.id = c.leadgen WHERE c.assigned_to = $adviser_id";
