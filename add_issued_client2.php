                      ...
                      <div class="col-sm-2">\
                          <label>Compliance Status\
                            <div class="input-group">\
                              ...
                              <select data-dc = "1" class="form-control compliance_status_options" name="compliance_status_' + dealsCount + '" id="compliance_status_' + dealsCount + '"/>\
                              </select>\
                            </div>\
                          </label>\
                        </div>\
...
<label>Existing Client
  ...
  <?php 
  $query =  "SELECT c.id, c.name, c.leadgen, c.assigned_to FROM submission_clients s LEFT JOIN clients_tbl c ON s.client_id = c.id WHERE c.status!='Cancelled' ORDER BY name ASC";
  $displayquery=mysqli_query($con,$query) or die('Could not look up user information; ' . mysqli_error($con));
...
$deals_op = json_encode($deals, JSON_HEX_APOS);
  ...
  if($first_issued_date!="0"){
    ...
    if(empty($errors)){
      ...
      echo "<script>alert('Submission Client successfully added!');
        window.location.replace('issued_client_profiles');
      </script>";
      header("Refresh:0");
      //ob_end_flush();
