...
if(count($report_data->issued_deals) > 0) {
	...
	*/
	...
	$pdf->SetWidths(array(30, 20, 20, 20, 20, 20, 20, 20, 30));
	$pdf->Row(array("Life Insured", "Policy #", "Co.", "Source", "Issue Date", "API", "Comp. Status", "Audit", "Notes"), false, array(224, 224, 224));
	...
	foreach ($report_data->issued_deals as $deal) {
		...
		$pdf->SetFillColor(224, 224, 224);
		$fill = (($ctr % 2) === 0) ? true : false;
		$pdf->Row(array($life_insured, $policy_number, $company, $source, NZEntryToDateTime($date), "$" . number_format($api, 2), $compliance_status, $audit_status, str_replace("<br>", "\r\n", $notes)), $fill, array(224, 224, 224));
		$ctr++;
	}
