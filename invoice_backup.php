...
$pdf->SetFont('Helvetica','',12);
$pdf->Write(0, date("d/m/Y", strtotime($invoice_date)));
...
$search_leads="SELECT * FROM clients_tbl WHERE assigned_to='$adviser_id' AND assigned_date<='$until' AND assigned_date>='$date_from' AND lead_by!='Telemarketer' AND status!='Cancelled'";
...
if($if_page2>0):
	...
	$searchclient ="SELECT * FROM clients_tbl WHERE id IN ('" . $wherestring . "') AND status='Seen'";
	...
	$wherestring = implode("','",$client_leads);
	$searchclient ="SELECT * FROM clients_tbl WHERE id IN ('" . $wherestring . "') AND status='Agreement'";
