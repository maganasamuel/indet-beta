 	...
 	<body>
 		<div align="center">
 			...
 			<?php require "database.php";
					...
					$summary_data = $adviserController->getAdviserPayableLeads($adviser_id);
					...
					//echo $query . "<hr>";
					//Fetch all of Adviser's KiwiSaver Data
					$total_kiwisaver_deals = 0;
					$total_kiwisaver_commission = 0;
					$total_kiwisaver_gst = 0;
					$total_kiwisaver_balance = 0;
					$clients->kiwisaver_deals = array();

					$dealController = new DealController();
					$kiwisaver_deals = $dealController->GetKiwiSaversIssuedByAdviser($adviser_id);
					...
					while ($row = $kiwisaver_deals->fetch_assoc()) {
						...
						$total_kiwisaver_commission += $row["commission"];
						$total_kiwisaver_gst += $row["gst"];
						$total_kiwisaver_balance += $row["balance"];

						$total_kiwisaver_deals++;
					}
 				...
 				<div class="row">
 					...
 					<div class="col-sm-1"><button data-toggle='tooltip' title='Create Invoice' type="button" class="btn btn-primary" style="margin-top:-50px;" id="create"><span style="font-size:15px;" class="glyphicon glyphicon-print"></span></button> <a href="edit_adviser.php?edit_id=<?php echo $adviser_id ?>" class="btn btn-warning" style="margin-top:-50px;"><span style="font-size:15px;" class="glyphicon glyphicon-pencil"></span></a></div>
 				</div>
 		</div>
								...
								if (isset($clients->submitted)) {
									if (count($clients->submitted) > 0) {
										...
										foreach ($clients->submitted as $submission_client) {
											$id = $submission_client["ClientID"];
											$client_name = $submission_client["Name"];
											$name = $submission_client["Client"];
											$sub_date = $submission_client["Date"];
											$sub_date_order = DateTimeToNZEntry($submission_client["Date"]);
											$cid = $submission_client["ID"];
											$sub_api = number_format($submission_client["SubmissionAPI"], 2);
											echo "
			<tr cellpadding='5px' cellspacing='5px'>
				<td><a target='_blank' href='submission_client_profiles?view=$id&name=$client_name'><i class='fa fa-search'></i></a></td>
				<td><a target='_blank' href='submission_client_profiles?edit=$id&name=$client_name'><i class='fa fa-pencil-alt text-warning'></i></a></td>
				<td>$name</td>
				<td data-order='$sub_date_order'>$sub_date</td>
				<td>$" . "$sub_api</td>
			</tr>
				";
										}
								...
								if (isset($clients->issued_deals)) {
									if (count($clients->issued_deals) > 0) {
										...
										foreach ($clients->issued_deals as $deal) {
											...
											$name = $deal["Client"];
											$date = $deal["Date"];
											$date_order = DateTimeToNZEntry($deal["Date"]);
											$cid = $deal["ID"];
											$api = number_format($deal["IssuedAPI"], 2);
											echo "
			<tr cellpadding='5px' cellspacing='5px'>
				<td><a target='_blank' href='issued_client_profiles?view=$id&name=$client_name'><i class='fa fa-search'></i></a></td>
				<td><a target='_blank' href='issued_client_profiles?edit=$id&name=$client_name'><i class='fa fa-pencil-alt text-warning'></i></a></td>
				<td>$name</td>
				<td data-order='$date_order'>$date</td>
				<td>$" . "$api</td>
			</tr>
				";
										}
 			...
 			<div class="row">
 				<div class="col-sm-3 text-center">
 					<h4>
 						Total KiwiSaver Deals: <br><?php echo $total_kiwisaver_deals ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-3 text-center">
 					<h4>
 						Total KiwiSaver Commission: <br>$<?php echo number_format($total_kiwisaver_commission, 2) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-3 text-center">
 					<h4>
 						Total KiwiSaver GST: <br>$<?php echo number_format($total_kiwisaver_gst, 2) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-3 text-center">
 					<h4>
 						Total KiwiSaver Balance: <br>$<?php echo number_format($total_kiwisaver_balance, 2) ?><br>
 					</h4>
 				</div>
 			</div>
 			<h2 class="sub-header" style="text-align:center;">KiwiSaver Deals</h2>
 			<div class="table-responsive">
 				<table data-toggle="table" class="table table-striped " cellpadding="5px" cellspacing="5px" style="width:90%;">
 					<thead>
 						<td>Edit</td>
 					...
 					<tbody>
 						<?php
								if (isset($clients->kiwisaver_deals)) {
									if (count($clients->kiwisaver_deals) > 0) {

										foreach ($clients->kiwisaver_deals as $deal) {
											$name = $deal["Client"];
											$date = $deal["Date"];
											$date_order = $deal["Date"];
											$date = NZEntryToDateTime($deal["Date"]);
											$commission_order = $deal["commission"];
											$commission = number_format($deal["commission"], 2);
											$gst_order = $deal["gst"];
											$gst = number_format($deal["gst"], 2);
											$balance_order = $deal["balance"];
											$balance = number_format($deal["balance"], 2);
											$kid = $deal["ID"];
											echo "
				<tr cellpadding='5px' cellspacing='5px'>
					<td><a target='_blank' href='kiwisavers?edit=$kid&name=$name'><i class='fa fa-pencil-alt text-warning'></i></a></td>
					<td>$name</td>
					<td data-order='$date_order'>$date</td>
					<td data-order='$commission_order'>\$$commission</td>
					<td data-order='$gst_order'>\$$gst</td>
					<td data-order='$balance_order'>\$$balance</td>
				</tr>
					";
										}
									}
	...
	";
	    ...
	    ";
 			...
 			<div class="row">
 				...
 				<div class="col-sm-5 text-center">
 					<h4>
 						Total Payable Leads: <br><?php echo count($summary_data->leads) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-5 text-center">
 					<h4>
 						Total Payable For Leads: <br>$<?php echo number_format((count($summary_data->leads) * 35) + ((count($summary_data->leads) * 35) * .15), 2) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-1"></div>
 			</div>
 			<h2 class="sub-header" style="text-align:center;">Payable Leads</h2>
 			<div class="table-responsive">
 				<table id="payable_leads_table" data-toggle="table" class="table table-striped " cellpadding="5px" cellspacing="5px" style="width:90%;">
 					<thead>
 						<td>Edit</td>
 						<td>Name</td>
 						<td>Date Assigned</td>
								...
								if (isset($summary_data->leads)) {
									if (count($summary_data->leads) > 0) {
										...
										while ($rows = $dataset->fetch_assoc()) {
											...
											$date_assigned_order = $rows["assigned_date"];
											$status = $rows["status"];

											echo "<tr id='payable_lead_$id' cellpadding='5px' cellspacing='5px'>
											<td><a target='_blank' href='clients?edit=$id&name=$client_name'><i class='fa fa-pencil-alt text-warning'></i></a></td>
											<td>$client_name</td>
											<td data-order='$date_assigned_order'>$date_assigned</td>
											<td>$status</td>
										</tr>";
										}
									}
 			...
 			<div class="row">
 				...
 				<div class="col-sm-5 text-center">
 					<h4>
 						Total Payable Issued Leads: <br><?php echo count($summary_data->issued) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-5 text-center">
 					<h4>
 						Total Payable For Issued Leads: <br>$<?php echo number_format(count($summary_data->issued) * 50 + ((count($summary_data->issued) * 50) * .15), 2) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-1"></div>
 			</div>
 			<h2 class="sub-header" style="text-align:center;">Payable Issued Leads</h2>
 			<div class="table-responsive">
 				<table data-toggle="table" class="table table-striped " cellpadding="5px" cellspacing="5px" style="width:90%;">
 					<thead>
 						<td>Name</td>
								...
								if (isset($summary_data->issued)) {
									if (count($summary_data->issued) > 0) {
										...
										while ($rows = $dataset->fetch_assoc()) {
											...
											echo "<tr cellpadding='5px' cellspacing='5px'>
											<td>$client_name</td>
											<td data-order='$date_assigned_order'>$date_assigned</td>
											<td>$status</td>
										</tr>";
 			...
 			<hr>
 			<?php
				$client_exempted_from_invoice = [];			
				if(count($summary_data->exempted_clients) > 0 || count($summary_data->exempted_issued_clients) > 0){
					$dataset = $clientController->getClientsInArray(array_merge($summary_data->exempted_clients, $summary_data->exempted_issued_clients));
	
					while ($row = $dataset->fetch_assoc()) {
						$client_exempted_from_invoice[] = $row;
					}
				}
			?>
 			<div class="row">
 				...
 				<div class="col-sm-5 text-center">
 					<h4>
 						Total Waived/Cancelled Leads: <br><?php echo count($client_exempted_from_invoice) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-5 text-center">
 					<h4>
 						Total Waived/Cancelled Leads Amount: <br>$<?php echo number_format(count($client_exempted_from_invoice) * 35 + ((count($client_exempted_from_invoice) * 35) * .15), 2) ?><br>
 					</h4>
 				</div>
 				<div class="col-sm-1"></div>
 			</div>
 			<h2 class="sub-header" style="text-align:center;">Waived/Cancelled Leads From Invoice</h2>
 			<div class="table-responsive">
 				<table id="exempted_clients_table" data-toggle="table" class="table table-striped " cellpadding="5px" cellspacing="5px" style="width:90%;">
 					<thead>
 						<td>Edit</td>
 						<td>Name</td>
 						<td>Date Updated</td>
 					...
 					<tbody>
 						<?php
								foreach ($client_exempted_from_invoice as $rows) {
									extract($rows);
									$date_status_updated = date("d/m/Y", strtotime($rows["date_status_updated"]));
									$date_status_updated_order = $rows["date_status_updated"];

									echo "<tr id='exempted_client_$id' cellpadding='5px' cellspacing='5px'>
											<td><a target='_blank' href='clients?edit=$id&name=$name'><i class='fa fa-pencil-alt text-warning'></i></a></td>
											<td>$name</td>
											<td data-order='$date_status_updated'>$date_status_updated</td>
											<td>$status</td>
										</tr>";
								}
 		...
 		<?php
 			...
 			<script>
 				$(document).ready(function() {
 					...
 					let payable_leads_table = $("#payable_leads_table").DataTable({
 						retrieve: true,
 						"aaSorting": [
 							[3, 'asc']
 						]
 					...
 					let exempted_clients_table = $("#exempted_clients_table").DataTable({
 						retrieve: true,
 						"aaSorting": [
 							[3, 'asc']
 						]
 					...
 					$(document).on("click", ".convert_to_waived", function() {
 						let client_id = $(this).data("id");
 						let status = "";

 						$.confirm({
 							title: 'Select an action',
 							content: 'You are about to exempt a client from the Invoice Summary',
 							buttons: {
 								waive: {
 									text: 'Waive Client',
 									btnClass: 'btn-orange',
 					...
 					function ExemptClient(client_id, status) {
 						let post_data = {
 							action: "exempt_client_from_invoice",
 							status: status,
 							client_id: client_id
 						};
 						...
 						$.ajax({
 							...
 							success: function(data) {
 								...
 								$.confirm({
 									title: 'Success!',
 									content: 'You have successfully exempted ' + data.name + ' from the Invoice Summary.',
 									buttons: {
 										Ok: function() {

 											var newData = {
 												"0": "<button class='btn btn-success remove_exemption' type='button' data-id='" + data.exemption_id + "'><i class='fas fa-refresh'></i></button>",
 												"1": "<a target='_blank' href='clients?edit=" + data.exemption_id + "&name=" + data.name + "'><i class='fa fa-pencil-alt text-warning'></i></a>",
 												"2": data.name,
 												"3": {
 													"display": data.date_assigned,
 													"@data-order": data.date_assigned_order
 												},
 												"4": data.exemption_status
 											};

 											exempted_clients_table.row.add(newData).node().id = "exempted_client_" + data.id;
 											exempted_clients_table.row("#exempted_client_" + data.id).data(newData).draw(false);

											 payable_leads_table.row("#payable_lead_" + data.client_id).remove().draw(false);
 										},
 									}
 								});
 							}
 					...
 					$(document).on("click", ".remove_exemption", function() {
 						let exemption_id = $(this).data("id");

 						$.confirm({
 							title: 'Select an action',
 							content: 'You are about to remove a client from being exempted from the Invoice Summary',
 							buttons: {
 								confirm: {
 									text: 'Remove Exception',
 									btnClass: 'btn-success',
 									action: function() {
										RemoveClientExemption(exemption_id);
 									}
 								},
 								cancel_action: {
 									text: 'Cancel Action',
 					...
 					function RemoveClientExemption(exemption_id) {
 						let post_data = {
						...
						console.log(post_data);
 						...
 						$.ajax({
 							...
 							success: function(data) {
 								...
 								$.confirm({
 									title: 'Success!',
 									content: 'You have successfully removed ' + data.name + ' from the exemptions list.',
 									buttons: {
 										Ok: function() {

 											var newData = {
 												"0": "<button class='btn btn-danger remove_exemption' type='button' data-id='" + data.id + "'><i class='fas fa-times-circle'></i></button>",
 												"1": "<a target='_blank' href='clients?edit=" + data.id + "&name=" + data.name + "'><i class='fa fa-pencil-alt text-warning'></i></a>",
 												"2": data.name,
 												"3": {
 													"display": data.date_assigned,
 													"@data-order": data.date_assigned_order
 											...
 											payable_leads_table.row.add(newData).node().id = "payable_lead_" + data.id;
 											payable_leads_table.row("#payable_lead_" + data.id).data(newData).draw(false);

 											exempted_clients_table.row("#exempted_client_" + exemption_id).remove().draw(false);
 										},
 									}
 								});
 							}
