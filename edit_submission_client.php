...
if (!isset($_SESSION["myusername"])) {
  ...
  if (isset($_POST["enter"])) {
    ...
    if (mysqli_query($con, $sql)) {
      ...
      header("Location:edit_submission_client.php?submission_id=$submission_id&success");
      //header("Refresh:0");
      //ob_end_flush();
  ...
  if (isset($_GET["submission_id"])) {
    ...
    if (isset($_GET["success"])) {
      echo "<script>
    alert('Submission Client successfully updated!');
    window.location = 'edit_submission_client.php?submission_id=$submission_id';
    </script>";
    }
  } else {
