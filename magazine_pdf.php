...
{
    var $uploads_folder = "../indet_photos_stash/";
    var $default_image = "images/default_pic.png";
    ...
    function Header1($y, $text, $fontSize = 23)
    {
    ...
    function Header2($y, $text, $fontSize = 23)
    {
        $this->SetFillColor(102, 163, 194);
        $this->SetDrawColor(102, 163, 194);
        $this->Rect(10.5, $y - 5, 195, 15, "DF");
    ...
    function FlexibleHeader1($x, $y, $w, $h, $text, $fontSize = 23)
    {
        $this->SetFillColor(102, 163, 194);
        $this->SetDrawColor(102, 163, 194);
    ...
    function BiMonthlyPage($advisers)
    {
    ...
    function CumulativePage($advisers)
    {
        //page 2 BiMonthly API
        $this->AddPage();
    ...
    function CumulativeRBAPage($advisers)
    {
        //page 2 BiMonthly API
        $this->AddPage();
    ...
    function BiMonthlyKiwiSaversPage($advisers)
    {
        //page 2 BiMonthly API
        $this->AddPage();
    ...
    function CumulativeKiwiSaversPage($advisers)
    {
        //page 2 BiMonthly API
        $this->AddPage();
    ...
    function BDMBiMonthlyPage($bdms)
    {
        //page 2 BiMonthly API
        $this->AddPage();
    ...
    function BDMCumulativePage($bdms)
    {
        //page 2 BiMonthly API
        $this->AddPage();
    ...
    function BDMPage($bdms, $quarterTitle)
    {
        //page 2 BiMonthly API
        $this->AddPage();
        ...
        */
            ...
            foreach ($bdms as $bdm) {
                if ($bdm["generated"] > 0) {
                    if($bdm["issued_api"] > 0 && $bdm["name"] != "Others"){
            //var_dump($bdm);
        ...
        function BDMKSPage($bdms, $quarterTitle)
        {
        //page 2 BiMonthly API
            $this->AddPage();
        ...
        */
        ...
        function RecordsPage($records)
        {
            foreach ($records as $index => $record) {
                ...
                //Total API
                    ...
                    $this->SetFont('Calibri', '', 20);
                    $this->Cell(120, 8, "", "", "0", "L");
                    $record_display = ($record["record_type"] == "Count") ? $record["record"] : "$" . number_format($record["record"], 2);
        ...
        function BirthdaysPage($persons)
        {
        //page 2 BiMonthly API
            $this->AddPage();
        ...
        function WorkAnniversariesPage($persons)
        {
        //page 2 BiMonthly API
            $this->AddPage();
        ...
        function AnnouncementsPage($announcement)
        {
        //page 2 BiMonthly API
            $this->AddPage();
    ...
    function Strings($advisers){
        $sorted_advisers = array();
        $t_array = array();
        $p_array = array();
        $g_array = array();
        $s_array = array();
        ...
        foreach(array_slice($sorted_advisers, 1) as $index =>  $adviser){
            if($index != 5){
                if($adviser["name"] != "Others" && $adviser["score"]){
                    if($adviser["score"] == "Titanium"){
                        $this->SetDrawColor(102, 163, 200);
                        $this->SetFillColor(102, 163, 194);
                    } else if($adviser["score"] == "Platinum") {
                        $this->SetDrawColor(207, 21, 21);
                         $this->setFillColor(212, 91, 91);
                    } else if($adviser["score"] == "Gold") {
                        $this->SetDrawColor(214, 181, 0);
                        $this->setFillColor(255, 239, 148);
                    } else {
                        $this->SetDrawColor(163, 163, 163);
    ...
    function WinnerScore($advisers){
        ...
        } else if($advisers[0]["deals"] >= 4 && $advisers[0]["issued_api"] >= 6000) {
            $this->SetFont('Calibri', 'B', 20);
            $this->Cell(50, 8, "Score: Platinum", "", "1", "L");
        } else if ($advisers[0]["deals"] >= 3 && $advisers[0]["issued_api"] >= 4500) {
            $this->SetFont('Calibri', 'B', 20);
            $this->Cell(50, 8, "Score: Gold", "", "1", "L");
        } else if ($advisers[0]["deals"] >= 2 && $advisers[0]["issued_api"] >= 2000){
            $this->SetFont('Calibri', 'B', 20);
...
function CreateMagazinePDF($magazine_data, $preview = true, $randomize_name = true)
{
    ...
    $blue = array(50, 143, 168);
    $darkblue = array(50, 91, 168);
    $pdf->SetY(78);
    $pdf->SetX(15);
    ...
    if (count($magazine_data->all_winner_score) > 0){
        if($magazine_data->all_winner_score[0]["name"] != "Others"){
            ...
            foreach($magazine_data->pages as $winner_string){
                if($winner_string["title"] == "Winner Strings"){
    ...
    if (count($magazine_data->rba_cumulative_advisers) > 0)
        if($magazine_data->rba_cumulative_advisers[0]["name"] != "Others")
        ...
        if (count($magazine_data->bi_monthly_bdms) > 0)
            if($magazine_data->bi_monthly_bdms[0]["name"] != "Others")
