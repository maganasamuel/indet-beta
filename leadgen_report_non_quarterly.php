...
$issued_clients_query = "SELECT *,i.issued,l.name as leadgen_name, i.name as issued_client_name, i.id as issued_client_id from leadgen_tbl l LEFT JOIN issued_clients_tbl i ON i.leadgen = l.id LEFT JOIN clients_tbl c ON i.name=c.id WHERE l.id ='$leadgen_id' AND i.date_issued <= '$until' AND i.date_issued >= '$date_from'";
...
//$invoice_number_date = substr($date_from,0,2).substr($date_from,0,2).substr($date_from,6,4);
...
$pdf->Cell(200, 10, $r_type . ' Performance Report', "0", "1", "C", 'true');

$pdf->SetFont('Helvetica', 'B', 15);
$pdf->Cell(17, 10, "Name:", "0", "0", "L");
$pdf->SetFont('Helvetica', '', 15);
$pdf->Cell(78, 10, "$leadgen_name", "0", "0", "L");
//$pdf->Cell(78,10,"$report_schedule_type","0","0","L");
$pdf->Cell(5, 10, '', "0", "0", "R");
$pdf->SetFont('Helvetica', 'B', 15);
...
if ($lead_by == "Face-to-Face Marketer") {
    include('leadgen_report_face_to_face_marketer.php');
} elseif ($lead_by == "Telemarketer") {
    include('leadgen_report_telemarketer.php');
}
$pdf->Ln(2);
...
if ($totalclients != 0 || $totalissuedclients != 0) {
	...
	$pool = "";
	$required_leads_type = $leads_required_type;
