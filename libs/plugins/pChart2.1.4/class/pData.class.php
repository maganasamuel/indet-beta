...
class pData
{
  ...
  /* Add a single point or an array to the given serie */
  function removeFromLabels($Values, $SerieName = "Labels")
  {
    if (!is_array($Values)) {
      $Value = $Values;
      $Values = array();
    ...
    foreach ($Values as $Key => $Value) {
      foreach ($this->Data["Series"][$SerieName]["Data"] as $index => $data) {
        if ($Value == $data)
          array_splice($this->Data["Series"][$SerieName]["Data"], $index, 1);
      }
    }
  }
  /* Strip VOID values */
