...
if($is_update=="No"){
    //Insert into record
    $sql="INSERT INTO leads_data (name,data,notes,agent_id) 
    VALUES ('$name','$lead_data','$notes',$agent_id)"; 
    ...
    $sql = "UPDATE leads_data SET name='$name', name='$name', lead_data='$lead_data' WHERE id=$leads_data_id";
    if ($result = mysqli_query($con,$sql)) {
    }else{
        echo $sql;
        $errors++;
