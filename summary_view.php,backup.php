...
//var_dump($_POST);										//Desc
$summary_data = json_decode($summary_data["data"]);
...
if($rowcount==0){
	$output = array();
	$output['status'] = "error";
	$output['responseText'] = "No 1Records Found";
	return print json_encode($output);
}


$summary_data->adviser = mysqli_fetch_assoc($result);
...
$pdf->SetFont('Helvetica','B',12);
$pdf->Cell(100,10,'Billed Assigned Leads', 0, 0,'L');
$pdf->Cell(43,10,' ', 0, 0,'R');
$pdf->SetFont('Helvetica','',12);
$pdf->Cell(60,10,$summary_data->total_billed_assigned_leads, 0, 1,'C');
//$pdf->Cell(55,10,'$'.desc_val($desc[0]), 0, 1,'R');

$pdf->SetXY($x+10, $y+137); 
$pdf->SetFont('Helvetica','B',12);
$pdf->Cell(100,10,'Billed Issued Leads', 0, 0,'L');
$pdf->Cell(43,10,' ', 0, 0,'R');
$pdf->SetFont('Helvetica','',12);
$pdf->Cell(60,10,$summary_data->total_billed_issued_leads, 0, 1,'C');
...
$pdf->MultiCell(100,11,$invoices_id_list,0,"L",false);
$pdf->Cell(60,0,'',"0","1","C");
...
if(count($summary_data->invoices_in_range)>0):
	...
	$pdf->Cell(35,6,"","T",0,'C');
	$pdf->Cell(30,6,$leadstotal,"T",0,'C');
	$pdf->Cell(30,6,$issuedtotal,"T",0,'C');
...
if(count($summary_data->invoice_transaction_histories)>0):
	...
	$pdf->Cell(100,7,'Descripion', 0,0,'L');
	...
	$amounttotal = 0;
	$remainingtotal = 0;
	$summary_data->invoice_transaction_histories = convertToArray($summary_data->invoice_transaction_histories);
...
if(count($summary_data->leads)>0):
	...
	$pdf->SetFillColor(224,224,224);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(100,10,'Leads', 0, 0,'L','true');
	$pdf->Cell(43,10,' ', 0, 0,'R','true');
	$pdf->Cell(43,10,' ', 0, 0,'R','true');
	$pdf->Cell(43,10,' ', 0, 0,'R','true');
	$pdf->Cell(60,10,'  ', 0, 1,'C','true');
	...
	$imploded_array = implode(",", $summary_data->leads);
	...
	$pdf->SetFont('Helvetica', 'B',12);
	$pdf->Cell(160, 7, 'Total Leads', "T",0,'L');
...
if(count($summary_data->issued)>0):
	...
	$pdf->SetFillColor(224,224,224);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(100,10,'Issued Leads', 0, 0,'L','true');
	$pdf->Cell(43,10,' ', 0, 0,'R','true');
	$pdf->Cell(43,10,' ', 0, 0,'R','true');
	$pdf->Cell(43,10,' ', 0, 0,'R','true');
	$pdf->Cell(60,10,'  ', 0, 1,'C','true');
	...
	$pdf->SetFont('Helvetica','B',12);
	$pdf->Cell(160,7,'Total Issued Leads', "T",0,'L');
	$pdf->Cell(35,7,$rowcount, "T",1,'L');
	$pdf->Ln();
endif;

$pdf->Output("I",$summary_data->adviser["name"] . ' Invoice Summary ');
