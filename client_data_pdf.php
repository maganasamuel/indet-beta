...
function CreateClientDataPDF($id, $reference_no, $preview = true)
{
    ...
    $pdf->Cell(96, 8, "Date Assigned: " . $indet_dates_helper->NZEntryToDateTime($client->assigned_date), "0", "1", "L", false);
    $pdf->Ln(1);

    $pdf->SetFillColor(223, 235, 247);
    $pdf->Rect(10, $pdf->GetY(), 100, 8, "F");
    $pdf->Rect(111, $pdf->GetY(), 96, 8, "F");
    $pdf->Cell(101, 8, "Date Generated: " . $indet_dates_helper->NZEntryToDateTime($client->date_submitted), "0", "0", "L", false);
    $pdf->Cell(96, 8, "Appointment Date : " . $indet_dates_helper->NZEntryToDateTime($client->appt_date), "0", "1", "L", false);
    $pdf->Ln(1);

    $pdf->SetFillColor(190, 215, 239);
    $pdf->Rect(10, $pdf->GetY(), 100, 8, "F");
    $pdf->Rect(111, $pdf->GetY(), 96, 8, "F");
    $pdf->Cell(101, 8, "City : " . $client->city, "0", "0", "L", false);
    $pdf->Cell(96, 8, "Zip code : " . $client->zipcode, "0", "1", "L", false);
    $pdf->Ln(1);
    ...
    $pdf->Rect(111, $pdf->GetY(), 96, 8, "F");
    $pdf->SetFont('Helvetica', '', 13);
    $pdf->Cell(101, 8, "Email: " . $client->email, "0", "0", "L", false);
    $pdf->Cell(96, 8, "Phone: " . $client->appt_time, "0", "1", "L", false);
    ...
    $pdf->Ln(1);
    $pdf->Row(array("", htmlspecialchars_decode($client->notes), ""), true, array(190, 215, 239));
    $pdf->Rect(10, $pdf->GetY(), 197, 1, "FD");
    $pdf->Ln(1);
    ...
    //$path="files/".$mix.".pdf";
    $path = "";
    $filename = str_replace("/"," or ", $client->name) . " Data.pdf";
...
function CreateLeadDataPDF($id, $reference_no, $preview = true)
{

    $clientController = new ClientController();

    $leads_data = $clientController->getLeadDataFromClientID($id)->fetch_assoc();
    $lead_data = json_decode($leads_data["data"]);
    $timestamp = $leads_data["timestamp"];
    //var_dump($lead_data);

    $leadGeneratorController = new LeadGeneratorController();
    $agent_data = $leadGeneratorController->getLeadGenerator($lead_data->agent_id)->fetch_assoc();
    ...
    $products_data = $productController->getAllProducts();
    ...
    $pdf->SetFillColor(223, 235, 247);
    $pdf->Rect(10, $pdf->GetY(), 197, 8, "F");
    $pdf->MultiCell(197, 8, "Address: " . $lead_data->client_address, "0", "1", "L", false);
    $pdf->Ln(1);
    ...
    $pdf->Rect(10,$pdf->GetY(),100,8,"F");
    $pdf->Rect(111,$pdf->GetY(),96,8,"F");
    $pdf->Cell(101,8,"Telephone: " . $lead_data->client_telephone,"0","0","L",false);
    $pdf->Cell(96,8,"Mobile: " . $lead_data->client_mobile,"0","1","L",false);
    $pdf->Ln(1);
    */
    $pdf->SetFillColor(223, 235, 247);
